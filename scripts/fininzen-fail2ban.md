...
see `fininzen/settings.py` → `DEFAULT_THROTTLE_RATES`). This document adds an
...
`/etc/fail2ban/filter.d/fininzen-auth.conf`:
...
`/etc/fail2ban/jail.d/fininzen-auth.conf`:
