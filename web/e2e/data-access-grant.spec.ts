...
async function createGrantViaApi(page: Page): Promise<void> {
    ...
    await page.request.post("/fininzen/api/auth/grants/", {
...
async function revokeAllGrantsViaApi(page: Page): Promise<void> {
    const accessToken = await getUserAToken(page);
    const res = await page.request.get("/fininzen/api/auth/grants/", {
        headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!res.ok()) {
        throw new Error(
    ...
    for (const g of given) {
        await page.request.delete(`/fininzen/api/auth/grants/${g.id}/`, {
            headers: { Authorization: `Bearer ${accessToken}` },
        });
    }
}
...
test.describe("DataAccessGrant UI", () => {
    ...
    test("4.6 — self-grant shows 400 error", async ({ page }) => {
        ...
        const res = await page.request.post("/fininzen/api/auth/grants/", {
            headers: { Authorization: `Bearer ${accessToken}` },
    ...
    // PARITY GAP: the view-as account <select> lives in AppHeader (AppChrome.tsx),
    // which the placeholder AppNav shell does not render yet. Re-enable once the
    // real header chrome (view-as dropdown + banner) is wired in. Tracked: task #8.
    test.fixme("4.1 — create grant via UI, recipient sees dropdown", async ({
        page,
    }) => {
        await loginAsTestUser(page, TEST_USER_A_EMAIL, TEST_USER_A_PASS);
    ...
    // PARITY GAP: view-as dropdown + "Viewing as" banner (AppChrome.tsx) not
    // rendered by the placeholder shell. Re-enable with the header chrome.
    test.fixme("4.2 — view-as shows yellow banner with owner email", async ({
        page,
    }) => {
        await loginAsTestUser(page, TEST_USER_A_EMAIL, TEST_USER_A_PASS);
        await createGrantViaApi(page);
    ...
    // PARITY GAP: depends on the view-as dropdown (AppChrome.tsx) absent from the
    // placeholder shell. Re-enable with the header chrome.
    test.fixme("4.5 — revoke grant removes option from recipient dropdown", async ({
        page,
    }) => {
        await loginAsTestUser(page, TEST_USER_A_EMAIL, TEST_USER_A_PASS);
        await createGrantViaApi(page);
