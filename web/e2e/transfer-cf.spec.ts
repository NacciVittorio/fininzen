...
test.describe("Transfer via Cash Flow form (K4.5)", () => {
    ...
    test("golden path: create transfer via Cash Flow form", async ({
        ...
        const accARes = await page.request.post("/fininzen/api/portfolio/", {
            headers,
            data: {
                name: ACCOUNT_A_NAME,
        ...
        const accBRes = await page.request.post("/fininzen/api/portfolio/", {
            headers,
            data: {
                name: ACCOUNT_B_NAME,
                investment_type: typeId,
        ...
        try {
            ...
            const cfRes = await page.request.get(
                "/fininzen/api/expenses/cashflow/",
                {
                    headers,
