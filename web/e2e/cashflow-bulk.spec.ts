...
async function cleanupOrphanedCats(page: Page, token: string): Promise<void> {
    ...
    const cats: Array<{ id: number; name: string }> = Array.isArray(body)
        ? body
        : (body?.results ?? []);
    for (const cat of cats.filter((c) => c.name === EXP_CAT_NAME)) {
...
test.describe("Cash Flow — Bulk selection", () => {
        ...
        try {
            ...
            const verifyPromise = page.waitForResponse(
                (res) =>
                    res
                        .url()
                        .includes("/fininzen/api/expenses/cashflow/bulk/") &&
            ...
            for (const id of [expA.id, expB.id, expC.id]) {
                const got = await (
                    await page.request.get(`/fininzen/api/expenses/${id}/`, {
                        headers,
                    })
                ).json();
            ...
            await page.request
                ...
                .catch(() => {});
            await page.request
                .delete(`/fininzen/api/expenses/categories/${expCat.id}/`, {
                    headers,
                })
                .catch(() => {});
        }
    ...
    test("bulk edit modal: 'Remove value' button swaps description input for pill", async ({
        ...
        try {
            ...
            await expect(
                page.locator(
                    '[data-testid="cf-bulk-field-cleared-description"]',
                ),
            ).toBeVisible();
    ...
    test("selection gating: outcome locks the kind; income click is rejected with a toast", async ({
        ...
        try {
            ...
            await page.request
                ...
                .catch(() => {});
            await page.request
                .delete(`/fininzen/api/expenses/categories/${incomeCat.id}/`, {
                    headers,
                })
                .catch(() => {});
        }
    ...
    test("bulk delete removes the selected rows", async ({ page }) => {
        ...
        try {
            ...
            await page
                .locator('[data-testid="cf-bulk-delete-confirm"]')
                .click();
