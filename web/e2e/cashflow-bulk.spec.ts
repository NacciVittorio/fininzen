...
test.describe("Cash Flow — Bulk selection", () => {
  ...
  test("bulk verify on selected expenses updates the API state", async ({
    ...
    try {
      ...
      await page.locator('[data-testid="cf-select-mode"]').click();
      await expect(
      ...
      const verifyPromise = page.waitForResponse(
        (res) =>
          res.url().includes("/fininzen/api/expenses/cashflow/bulk/") &&
      ...
      for (const id of [expA.id, expB.id, expC.id]) {
        const got = await (
          await page.request.get(`/fininzen/api/expenses/${id}/`, { headers })
        ).json();
        expect(got.is_verified).toBe(true);
      }
    } finally {
      await page.request
        .delete(`/fininzen/api/expenses/${expA.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/${expB.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/${expC.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/categories/${expCat.id}/`, { headers })
        .catch(() => {});
    }
  });
  ...
  test("bulk edit modal: 'Remove value' button swaps description input for pill", async ({
    ...
    try {
      ...
      await page.request
        .delete(`/fininzen/api/expenses/${exp.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/categories/${expCat.id}/`, { headers })
        .catch(() => {});
  ...
  test("selection gating: outcome locks the kind; income click is rejected with a toast", async ({
    ...
    try {
      ...
      await page.request
        .delete(`/fininzen/api/expenses/${expOut.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/${expIn.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/categories/${expCat.id}/`, { headers })
        .catch(() => {});
      await page.request
        .delete(`/fininzen/api/expenses/categories/${incomeCat.id}/`, { headers })
        .catch(() => {});
    }
  });
  ...
  test("bulk delete removes the selected rows", async ({ page }) => {
    ...
    try {
      ...
      const got = await page.request.get(`/fininzen/api/expenses/${exp.id}/`, {
        headers,
      });
