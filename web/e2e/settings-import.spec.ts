...
async function createBankAccount(
    ...
    const accRes = await page.request.post("/fininzen/api/portfolio/", {
        headers,
        data: {
            name: ACCOUNT_NAME,
...
async function deleteExpenseByDesc(
    ...
    const res = await page.request.get("/fininzen/api/expenses/", {
        headers: { Authorization: `Bearer ${token}` },
    });
    ...
    for (const exp of items) {
        if (exp.description === desc) {
            await page.request.delete(`/fininzen/api/expenses/${exp.id}/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
        }
    }
    ...
    test("CSV import with CategorySelect assigns category to imported rows", async ({
        ...
        if (catsRes.ok()) {
            ...
            for (const c of existing.filter((c) => c.name === CAT_NAME)) {
                await page.request.delete(
                    `/fininzen/api/expenses/categories/${c.id}/`,
                    {
                        headers,
                    },
                );
        ...
        const accsRes = await page.request.get("/fininzen/api/portfolio/", {
            headers,
        });
        if (accsRes.ok()) {
        ...
        await Promise.all([
            page.waitForResponse(
                (r) =>
                    r.url().includes("/fininzen/api/expenses/import-csv/") &&
                    r.ok(),
            ),
            importBtn.click(),
        ]);
        ...
        const expsRes = await page.request.get("/fininzen/api/expenses/", {
            headers,
        });
