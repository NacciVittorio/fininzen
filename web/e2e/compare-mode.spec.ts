...
test.describe("Compare mode — Monthly Net Worth", () => {
    ...
    // PARITY GAP (not a data-layer regression): dashboard prefs are
    // server-authoritative — on reload the profile load applies the server's
    // dashboard_preferences over the local cache. Demo users do NOT persist
    // dashboard_preferences server-side (the /auth/profile/ PATCH never fires for
    // demo), so compare mode reverts to the server default "single" after reload.
    // Re-enable with a non-demo user, or once demo prefs persist server-side.
    test.fixme("mode and years persist after page reload", async ({ page }) => {
