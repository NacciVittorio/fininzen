...
async function cleanupOrphanedCats(page: Page, token: string): Promise<void> {
  ...
  for (const cat of cats.filter((c: any) => [INC_CAT_NAME, EXP_CAT_NAME].includes(c.name))) {
    await page.request.delete(`/fininzen/api/expenses/categories/${cat.id}/`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  }
}
...
test.describe('Cash Flow — All Transactions', () => {
  ...
  test('view loads expenses on open', async ({ page }) => {
    ...
    const incCat = await (await page.request.post('/fininzen/api/expenses/categories/', { headers, data: { name: INC_CAT_NAME, color: '#4ade80', icon: '💚', category_type: 'income' } })).json();
    const expCat = await (await page.request.post('/fininzen/api/expenses/categories/', { headers, data: { name: EXP_CAT_NAME, color: '#ff6b6b', icon: '🔴', category_type: 'expense' } })).json();

    const incDesc = `E2E cf-income ${Date.now()}`;
    const expDesc = `E2E cf-expense ${Date.now() + 1}`;
    const inc = await (await page.request.post('/fininzen/api/expenses/', { headers, data: { description: incDesc, amount: '100.00', category: incCat.id, date: today } })).json();
    const exp = await (await page.request.post('/fininzen/api/expenses/', { headers, data: { description: expDesc, amount: '50.00', category: expCat.id, date: today } })).json();
  ...
  test('category filter restricts list', async ({ page }) => {
    ...
    try {
      ...
      await page.locator(`[data-testid="category-select-dropdown"] button:has-text("${EXP_CAT_NAME}")`).first().click();
      await page.click('[data-testid="category-select-trigger"]');
