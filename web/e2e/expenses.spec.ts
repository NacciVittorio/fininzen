...
async function goToCashFlow(page: Page): Promise<void> {
    await page.reload();
    // Not `.app-net-worth`: this helper is called more than once per test, and a
    // later call reloads while already on /cashflow, where that element doesn't
    // exist. The nav bar is part of the shared app shell on every route.
...
test.describe("Expenses CRUD", () => {
    ...
    test("add expense appears in list", async ({ page }) => {
        ...
        // Fill form. Category before Amount: leaving a focused Amount field (its
        // operator bar tears down on blur and reflows the sheet — see
        // AmountCalculator.tsx) mid-click can eat the very click that opens this
        // dropdown, on this narrow viewport.
        await page.locator('[data-testid="category-select-trigger"]').click();
        await page
            .locator(
                `[data-testid="category-select-dropdown"] button:has-text("${CAT_NAME}")`,
            )
            .first()
            .click();
        ...
        await Promise.all([
            page.waitForResponse(
                (r) => r.url().includes("/fininzen/api/expenses/") && r.ok(),
            ),
