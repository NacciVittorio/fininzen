...
import type { MnwAsset, MnwMonthlyValues, MnwOverview } from "./mnwTypes";
...
export default function MnwAssetDetailSheet({
    ...
    const [values, setValues] = useState<MnwMonthlyValues | null>(null);
    const [loading, setLoading] = useState(false);
    const cache = useRef<Record<number, MnwOverview>>({});
    ...
    const chartValues = (values || []).filter((v): v is number => v != null);
                ...
                >
                    ...
                    >
                        <button
                            type="button"
                            onClick={() =>
                                canPrev && setShownYear(shownYear - 1)
                            }
                            disabled={!canPrev}
                        ...
                        <button
                            type="button"
                            onClick={() =>
                                canNext && setShownYear(shownYear + 1)
                            }
                            disabled={!canNext}
