...
import type {
    AllocationTargetRow as AllocationTargetRowBase,
    RegroupedAllocationTarget,
} from "../../utils/allocationGroups";

type AllocRow = AllocationTargetRowBase & {
    id: number | string;
    name?: string;
    color?: string | null;
};
type RegroupedAllocRow = RegroupedAllocationTarget<AllocRow>;
...
export default function AllocationTargetsPanel({
    ...
    allocationData: readonly AllocRow[];
    regroupedAlloc: readonly RegroupedAllocRow[];
    allocChartType: string;
    setAllocChartType: (value: string) => void;
    allocGroup: string;
    setAllocGroup: (value: string) => void;
    setTab: (tab: string) => void;
    setSettingsMenu: (menu: string) => void;
    T: Translator;
}) {
...
function AllocationTargetPie({
    ...
    const pieData = regroupedAlloc
        ...
        .map((a) => ({
            total: a.current_pct,
            category__color: a.color ?? undefined,
...
function AllocationTargetRow({
    ...
    item: RegroupedAllocRow;
    T: Translator;
}) {
    const actionTone = item.action
        ? { buy: "accent", sell: "warning", ok: "neutral" }[item.action]
        : "neutral";
                        ...
                        {(a.target_pct ?? 0) > 0 && (
                            <div
                                style={{
                                    ...
                                    bottom: -2,
                                    width: 2,
                                    left: `${Math.min(a.target_pct ?? 0, 100)}%`,
                                    background: "var(--fg)",
                                    borderRadius: 1,
                                }}
                            />
