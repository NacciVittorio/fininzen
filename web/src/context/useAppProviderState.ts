...
export function useAppProviderState() {
    ...
    const [dashConfig, setDashConfig] = useState<DashboardSection[]>(() => {
        // SSR: localStorage is unavailable during Next's server render. Fall back
        // to the canonical config; the persisted layout hydrates on the client.
        if (typeof window === "undefined") return cloneDashConfig();
        try {
    ...
    const [wealthMetrics, setWealthMetrics] = useState<WealthMetric[]>(() => {
        if (typeof window === "undefined") return ["wealth"];
        try {
