...
export function filterExpensesByCategories<Row extends CategorizedRecord>(
  ...
  if (!Array.isArray(categoryIds) || categoryIds.length === 0) return expenses;

  const selectedIds = new Set(
    categoryIds.map((id) => parseInt(String(id), 10)),
  );
  return expenses.filter((expense) => {
    if (expense.category != null && selectedIds.has(expense.category)) return true;
...
export function groupAssets<Row extends AssetRecord>(assets: readonly Row[]): {
  bankAccounts: Row[];
...
export function buildKpiData(
  ...
  const monthlyExp =
    expenseSummary?.by_category
      ...
      .reduce(
        (sum, category) => sum + Number.parseFloat(String(category.total || 0)),
        0,
      ) || 0;
  const monthlyInc =
    expenseSummary?.by_category
      ?.filter((category) => category.category__category_type === "income")
      .reduce(
        (sum, category) => sum + Number.parseFloat(String(category.total || 0)),
        0,
      ) || 0;
  ...
  const returnRate = totalInvested
    ? (totalGain / totalInvested) * 100
    : 0;
  const expenseRatio = totalCurrent
    ? (monthlyExp / totalCurrent) * 100
    : 0;
