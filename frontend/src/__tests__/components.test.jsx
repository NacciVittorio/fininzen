...
describe("Modal", () => {
  ...
  it("does not call onClose when inner panel is clicked", () => {
    ...
    act(() => {
      innerPanel.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
...
describe("CategorySelect", () => {
  ...
  it("expands children when parent with subs is clicked", () => {
    ...
    const foodBtn = Array.from(buttons).find(
      (b) => b.textContent.includes("Food") && !b.textContent.includes("Pizza"),
    );
    act(() =>
  ...
  it("calls onChange when a leaf category is selected", () => {
    ...
    // Click Pizza (child)
    const pizzaBtn = buttons().find((b) => b.textContent.includes("Pizza"));
    act(() =>
...
describe("AssetCard", () => {
  ...
  it("shows ticker when present", () => {
    const a = { ...baseAsset, ticker: "VWCE.DE" };
  ...
  it("applies success color token for positive gain", () => {
    const a = { ...baseAsset, gain: 100, gain_percent: 10 };
    const c = renderTracked(<AssetCard a={a} onDelete={() => {}} T={T} />);
    // The return cell applies the --success token as inline color when gain >= 0
    const positiveEl = Array.from(c.querySelectorAll("[style]")).find(
      (el) => el.style.color === "var(--success)",
    );
...
describe("PieChart", () => {
  it("renders a full circle for a single 100% slice (no degenerate arc)", () => {
    ...
    const c = renderTracked(
      <PieChart
        data={[
          { total: 100, category__color: "#abcdef", category__name: "EUR" },
        ]}
      />,
    );
    // No wedge paths…
  ...
  it("renders one wedge path per slice when there are multiple", () => {
    const c = renderTracked(
      <PieChart
        data={[
          { total: 50, category__color: "#111111", category__name: "EUR" },
          { total: 50, category__color: "#222222", category__name: "USD" },
        ]}
      />,
    );
