...
function DashboardPreferenceWriter({ onDone }) {
  ...
  useEffect(() => {
    ...
    onDone?.();
  }, [bootstrapReady, onDone, toggleWealthMetric, updateMonthlyOverviewPrefs]);
  return null;
}

async function waitUntil(assertion, attempts = 200) {
  let lastError;
...
describe("authenticated bootstrap", () => {
  ...
  it("renders the dashboard while one aggregate trend request is pending", async () => {
    ...
    const mockFetch = vi
      ...
      .mockImplementation((url) => {
        ...
        if (path.endsWith("/expenses/trends/")) return trends.promise;
        ...
        if (path.endsWith("/auth/grants/")) {
          return Promise.resolve(jsonResponse({ given: [], received: [] }));
        }
        if (path.includes("/monthly-overview/")) {
          return Promise.resolve(
            jsonResponse({ available_years: [], assets: [], summary: {} }),
          );
        }
  ...
  it("hydrates Monthly Net Worth prefs from the profile and fetches that year", async () => {
    ...
    const mockFetch = vi
      ...
      .mockImplementation((url) => {
        ...
        if (path.endsWith("/auth/profile/")) {
          return Promise.resolve(
            jsonResponse({
              ...
              dashboard_config: [{ id: "wealth_trend", visible: true }],
              dashboard_preferences: {
                monthly_overview: { mode: "single", year: 2023, monthRange: 6 },
        ...
        if (path.includes("/monthly-overview/")) {
          return Promise.resolve(
            jsonResponse({ available_years: [2023], assets: [], summary: {} }),
          );
        }
        return Promise.resolve(jsonResponse([]));
      });
  ...
  it("resets stale local dashboard prefs when the profile has empty server prefs", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("wealthChartMetrics"))).toEqual([
      "wealth",
    ]);
    expect(JSON.parse(localStorage.getItem("dashConfig"))[0]).toEqual({
      id: "wealth_trend",
      visible: true,
    });
  });
  ...
  it("coalesces rapid dashboard preference updates into one complete patch", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation((url, options = {}) => {
      const path = String(url);
      if (path.endsWith("/auth/profile/") && options.method === "PATCH") {
      ...
      if (path.includes("/monthly-overview/")) {
        return Promise.resolve(
          jsonResponse({ available_years: [2024], assets: [], summary: {} }),
        );
      }
      return Promise.resolve(jsonResponse([]));
    });
    ...
    expect(patches[0].dashboard_preferences.monthly_overview).toMatchObject({
      mode: "single",
      year: 2024,
      monthRange: 12,
    });
