 ...
 */
function makeApiFetch(mockFetch, logoutFn, store) {
  const refreshingRef = { current: null };
    ...
    if (res.status === 401) {
      if (!refreshingRef.current) {
        refreshingRef.current = (async () => {
          try {
            const refreshRes = await mockFetch("/api/auth/token/refresh/");
            if (refreshRes.ok) {
...
describe("token refresh singleton", () => {
  it("chiama /token/refresh/ una sola volta per 15 richieste concorrenti con 401", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 15 }, (_, i) => apiFetch(`/api/endpoint-${i}/`)),
    );
  ...
  it("chiama logout una sola volta se il refresh fallisce con 401", async () => {
    ...
    await Promise.all(
      Array.from({ length: 10 }, (_, i) => apiFetch(`/api/endpoint-${i}/`)),
    );
