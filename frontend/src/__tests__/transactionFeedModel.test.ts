...
describe("transaction feed model", () => {
    const translate: Translator = (key) => key;
    const months = [
        "Jan",
        "Feb",
        "Mar",
    ...
    it("uses verified rows when a server summary is unavailable", () => {
        expect(
            getCashflowTotals([
                { date: "", type: "income", amount: "100", is_verified: true },
                { date: "", type: "outcome", amount: "30", is_verified: true },
                { date: "", type: "outcome", amount: "90", is_verified: false },
            ]),
    ...
    it("recognizes all, year and month periods", () => {
        expect(getCashflowPeriod({ date_from: "", date_to: "" })).toEqual({
            kind: "all",
        });
        expect(
            getCashflowPeriod({
