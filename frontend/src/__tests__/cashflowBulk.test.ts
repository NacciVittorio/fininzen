...
describe("CF bulk selection isSelected predicate", () => {
    ...
    function isSelected(
        id: string,
        {
            selectedIds,
            selectAllFiltered,
        }: { selectedIds: Set<string>; selectAllFiltered: boolean },
    ): boolean {
    ...
    it("explicit selection: only ids in the set are selected", () => {
        ...
        expect(
            isSelected("expense_1", {
                selectedIds: sel,
                selectAllFiltered: false,
            }),
        ).toBe(true);
        expect(
            isSelected("expense_3", {
                selectedIds: sel,
                selectAllFiltered: false,
            }),
        ).toBe(false);
...
describe("CF bulk selectionTypes derivation", () => {
    ...
    type SelectionItem = { id: string; source_type: string };
    function deriveTypes({
        ...
        items.forEach((item) => {
            ...
            const key =
                item.source_type === "expense" ? "expense" : item.source_type;
            out[key] = true;
        });
        return out;
    }
...
describe("CF bulk patch builder (keep/clear model — Iter 2)", () => {
    ...
    type TextField = { value: string; cleared: boolean };
    type PatchFields = {
        verified?: "keep" | "on" | "off";
        date?: string;
    ...
    function selectIdFromState(value: string): number | null | undefined {
...
describe("CF bulk selection kind gating", () => {
    ...
    type SelState = { ids: Set<string>; kind: string | null; tick: number };
    function makeReducer(): SelState {
        return { ids: new Set<string>(), kind: null, tick: 0 };
    }
    function toggle(state: SelState, id: string, itemType: string): SelState {
        if (state.ids.has(id)) {
...
describe("CF bulk error code mapping", () => {
    ...
    function format(
        data: { error_codes?: string[]; errors?: string[] },
        T: (key: string) => string,
    ): string {
    ...
    const T = (k: string) => translations.it[k] || k;
