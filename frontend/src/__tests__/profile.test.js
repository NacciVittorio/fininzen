 ...
 * Mirrors the pattern in tokenRefresh.test.js: extract pure logic, no React harness.
 */

function makeFetchProfile(apiFetch, setDecimalSeparator) {
...
function makeUpdateDecimalSeparator(apiFetch, setDecimalSeparator) {
  return async function updateDecimalSeparator(sep) {
    try {
      const res = await apiFetch("/api/auth/profile/", {
        method: "PATCH",
