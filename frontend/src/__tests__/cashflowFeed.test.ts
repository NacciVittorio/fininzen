...
describe("CF feed i18n keys", () => {
    ...
    it("en and it cf keys have non-empty values", () => {
        for (const key of CF_KEYS) {
            expect(
                translations.en[key]!.length,
                `empty en.${key}`,
            ).toBeGreaterThan(0);
            expect(
                translations.it[key]!.length,
                `empty it.${key}`,
            ).toBeGreaterThan(0);
        }
    });
...
describe("CF type filter logic", () => {
    ...
    function toggleType(currentTypes: string[], type: string): string[] {
...
describe("CF verified filter logic", () => {
    function buildParams(verified: boolean | null) {
...
describe("Portfolio transaction filter logic", () => {
    const buildParams = (
        filters: Parameters<typeof buildAssetTxQueryParams>[0],
    ...
    it("drops non-portfolio transaction types", () => {
        expect(buildParams({ types: ["cash_in", "sell"] })).toContain(
            "type=sell",
        );
    });
});
...
describe("Portfolio bulk selection logic", () => {
    function isSelected(
        id: number,
        {
            selectedIds,
            selectAllFiltered,
        }: { selectedIds: Set<number>; selectAllFiltered: boolean },
    ): boolean {
    ...
    it("select-all-filtered treats selected ids as exclusions", () => {
        ...
        expect(isSelected(1, { selectedIds, selectAllFiltered: true })).toBe(
            true,
        );
        expect(isSelected(2, { selectedIds, selectAllFiltered: true })).toBe(
            false,
        );
        expect(
            computeCount({
...
describe("CF delete response guard", () => {
    function shouldUpdateUI(res: { ok: boolean; status?: number }): boolean {
        return res.ok;
    }
...
describe("CF modal-close refresh behavior", () => {
    function shouldRefreshAfterModalTransition(
        wasOpen: boolean,
        isOpen: boolean,
    ): boolean {
