...
export default function BulkEditModal({ onClose }: { onClose: () => void }) {
    const {
        T,
        categories,
        bankAccounts,
    ...
    const kindLabel = T(
        (cfSelectionKind && kindKeys[cfSelectionKind]) ||
    ...
    return (
        <Modal title={title} onClose={handleClose}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <Field
                    label={T("cf_bulk_field_verified")}
                    error={fieldErrors.verified}
                >
                    <SegmentedControl
