...
function Field({ label, value }) {
  return (
    ...
    >
      <span style={{ color: "var(--fg-soft)", fontSize: 14 }}>{label}</span>
      <span
        style={{
          color: "var(--fg)",
          fontSize: 14,
          ...
          >
            {categoryText && (
              <Field
                ...
                value={`${data.category?.icon ? data.category.icon + " " : ""}${categoryText}`}
              />
            )}
            {accountText && (
              <Field
                label={isTransfer ? T("cf_detail_route") : T("account_label")}
                value={accountText}
              />
            )}
            <Field label={T("cf_edit_date")} value={formatDate(data.date)} />
            {canVerify && (
              <div
                style={{
                  display: "flex",
