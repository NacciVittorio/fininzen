...
type CmpAsset = {
    ...
    name: string;
    currency?: string;
    type: MnwInvestmentType | null | undefined;
...
type CmpMergedAsset = {
    ...
    currency?: string;
    type: MnwInvestmentType | null | undefined;
    A: MnwMonthlyValues | null;
    B: MnwMonthlyValues | null;
};
...
export function MnwCompareGrid({
    ...
    const assetsA: CmpAsset[] = (overviewA?.assets || []).map((a) => ({
        id: a.id + "_A",
        name: a.name,
        currency: a.currency,
    ...
    const assetsB: CmpAsset[] = (overviewB?.assets || []).map((a) => ({
        id: a.id + "_B",
        name: a.name,
        currency: a.currency,
        type: a.investment_type,
                ...
                <div className="data-scroll" style={{ overflowX: "auto" }}>
                    ...
                    >
                        ...
                        <tbody>
                            {Object.values(typeMap).map(
                                ({ type, assetsA: rowsA, assetsB: rowsB }) => {
                                    ...
                                    for (const a of rowsB) {
                                        ...
                                        merged.B = a.monthly_values ?? null;
                                    }
                                    return (
                                        <React.Fragment
                                            key={`group-${type?.id}`}
                                        >
                                            ...
                                            {Object.values(assetsByName).map(
                                                (asset) => (
                                                    <tr
                                                        key={asset.name}
                                                        style={{
                            ...
                            {getSummaryRows(T).map(
                                ({ key, label, isDelta, isPercent }) => (
                                    <tr
                                        key={key}
                                        style={{
                                            borderTop:
