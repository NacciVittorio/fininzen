...
export default function MnwAssetList({
    ...
    fetchOverviewForYear: (year: number) => Promise<MnwOverview | null>;
    T: Translator;
}) {
    ...
    const [detailAsset, setDetailAsset] = useState<MnwAsset | null>(null);
    ...
    const typeGroups = useMemo<AssetTypeGroup[]>(() => {
        const map: Record<string | number, AssetTypeGroup> = {};
    ...
    const deltaFor = (values: MnwMonthlyValues | undefined) => {
    ...
    const summaryRows: AssetListSummaryRow[] = [
        {
            key: "balance",
    ...
    const renderSummaryValue = (row: AssetListSummaryRow) => {
