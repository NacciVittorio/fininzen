...
export function MnwSingleGrid({
    ...
    const assetDataByYear: Record<
        string | number,
        Record<number, MnwMonthlyValues | undefined>
    > = {};
    ...
    if (prevYearOverview) {
        for (const asset of prevYearOverview.assets) {
            const entry =
                assetDataByYear[asset.id] ?? (assetDataByYear[asset.id] = {});
            entry[year - 1] = asset.monthly_values;
        }
    }

    // Build summary lookup: year → summary
    const summaryByYear: Record<number, MnwSummary> = {
        [year]: monthlyOverview.summary,
    };
    ...
    const typeMap: Record<string | number, TypeGroup> = {};
