...
function colValue(csvParsed, row, header) {
...
function normalizeImportType(value) {
  ...
  if (["income", "entrata", "entrate", "incoming", "in"].includes(normalized)) {
    return "income";
  }
  if (
    [
...
function parseCsvSignedAmount(value) {
...
function findCategoryByName(categories, name, categoryType) {
...
function findAccountByName(bankAccounts, name) {
  const normalizedName = normalizeCategoryName(name);
  if (!normalizedName) return null;
  return (
    bankAccounts.find(
...
export function buildCashflowImportRows({
  ...
  return csvParsed.rows
    .map((row) => {
      ...
      const isExpense = csvSignConv === "neg" ? rawAmount < 0 : rawAmount > 0;
      const categoryType =
        rowType ||
        (Number.isFinite(rawAmount) ? (isExpense ? "expense" : "income") : "");
      ...
      return {
        ...
        linked_asset: matchedAccount?.id ? String(matchedAccount.id) : "",
    ...
    .filter(
      (row) =>
        row && (row.description || row.category_name || row.amount || row.date),
    );
}
