export const localeFromSeparator = (sep) => (sep === "." ? "en-US" : "it-IT");
...
export function makeEurFormatters(locale) {
  ...
  return {
    formatEur: (n) => fmt.format(n ?? 0),
    formatEurFull: (n) => fmtFull.format(n ?? 0),
    formatEurCompact: (n) => fmtCompact.format(n ?? 0),
  };
}
...
export const makeFormatTick = (range) => (v) => {
...
export const filterAmountInput = (val) => {
...
function _normalizeDecimalString(value, sep) {
...
export const parseAmount = (val, sep = ",") => {
...
export const parseFlexibleDecimal = (value) => {
...
export const parseMoneyToString = (val, sep = ",") =>
  _normalizeDecimalString(val, sep);

export const isValidAmount = (val, sep = ",") => {
