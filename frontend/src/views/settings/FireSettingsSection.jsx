...
export function FireSettingsSection({ T, fetchFireGoal }) {
  const { apiFetch } = usePortfolio();
  const [form, setForm] = useState(DEFAULT_FIRE_FORM);
  ...
  const save = async () => {
    setSaveError(null);
    try {
      await saveFireSettings(apiFetch, form);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <FireFieldGrid T={T} fields={CORE_FIRE_FIELDS} form={form} set={set} />
      <button
        className="btn"
...
function FireFieldGrid({ T, fields, form, set }) {
  return (
    <div
