...
export function SharingSection({ T }) {
...
function SharingGivenList({ T, given, handlePermChange, handleRevoke }) {
  return (
    <>
          ...
          given.map((grant) => (
            ...
            >
              ...
              <select
                ...
                onChange={(event) =>
                  handlePermChange(grant.id, event.target.value)
                }
                style={{ fontSize: 11, padding: "2px 6px", minWidth: 110 }}
              >
                <option value="read">{T("permission_read")}</option>
                <option value="write">{T("permission_write")}</option>
                <option value="full">{T("permission_full")}</option>
...
function SharingReceivedList({ T, received }) {
  return (
    <>
      <div className="grouped-list__title">{T("share_received_title")}</div>
      <div className="grouped-list">
        {received.length ? (
          received.map((grant) => (
            ...
            >
              <span style={{ flex: 1, fontSize: 13 }}>{grant.owner_email}</span>
              <span
                style={{
                  fontSize: 11,
                  color: "var(--fg-soft)",
