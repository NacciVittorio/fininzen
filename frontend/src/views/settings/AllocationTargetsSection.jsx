...
export function AllocationTargetsSection() {
  const { T, investmentTypes, allocationData, apiFetch, fetchAllocationData } =
    useSettings();
...
function AllocationTargetGroup({ T, group, apiFetch, fetchAllocationData }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div
        style={{ fontSize: 12, color: "var(--fg-soft)", textAlign: "right" }}
      >
        {T("alloc_total")}:{" "}
        {group.rows
...
function AllocationTargetRow({ T, item, apiFetch, fetchAllocationData }) {
  return (
        ...
        {item.target_pct !== null && (
          <button
            ...
            onClick={async () => {
              ...
              await deleteAllocationTarget(apiFetch, item.target_id);
              fetchAllocationData();
            }}
          >
            x
...
function AllocationTargetProgress({ T, item }) {
  const actionColor = {
    buy: "var(--success)",
    sell: "var(--danger)",
    ok: "var(--accent)",
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div className="between" style={{ marginTop: 4 }}>
        <span style={{ fontSize: 10, color: "var(--fg-soft)" }}>0%</span>
        {item.action && (
          <span style={{ fontSize: 10, fontWeight: 700, color: actionColor }}>
            {T(`alloc_action_${item.action}`)} {Math.abs(item.diff).toFixed(1)}%
          </span>
        )}
        <span style={{ fontSize: 10, color: "var(--fg-soft)" }}>100%</span>
      </div>
    </div>
  );
}
