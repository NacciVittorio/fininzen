    ...
    <div className="card">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {fields.map(({ field, label, required }) => (
          <div key={field}>
            <div
              style={{ fontSize: 12, color: "var(--fg-soft)", marginBottom: 4 }}
            >
              {label}
              {required ? " *" : ""}
            </div>
            <select
              ...
              value={csvMap[field] || ""}
              onChange={(e) => {
                setCsvMap((prev) => ({ ...prev, [field]: e.target.value }));
                ...
                style={{
                  ...
                  background:
                    csvSignConv === val
                      ...
                      : "var(--card-inset)",
                  color:
                    csvSignConv === val ? "var(--accent)" : "var(--fg-soft)",
                  borderColor:
                    csvSignConv === val ? "var(--accent-ring)" : "var(--rule)",
                }}
              >
                {label}
              </button>
...
function CashflowFallbackCategories({
  ...
  const updateFallbackCategory = (field, value) => {
