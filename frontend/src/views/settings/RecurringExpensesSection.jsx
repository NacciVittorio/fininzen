...
export function RecurringExpensesSection({ setDeleteRecurringTarget }) {
  const {
    T,
    categories,
    assets,
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 13, color: "var(--fg-soft)", marginBottom: 16 }}>
        {T("recurring_desc")}
      </div>
          ...
          {recurringExpenses.map((recurringExpense) => (
            <RecurringExpenseRow
              ...
              category={categories.find(
                (category) => category.id === recurringExpense.category,
              )}
              linkedAccount={assets.find(
                (asset) => asset.id === recurringExpense.linked_asset,
              )}
      ...
      <div className="between">
        <div>
          ...
          >
            ...
            {recurringExpense.start_date && ` · ${recurringExpense.start_date}`}
            {recurringExpense.end_date && ` -> ${recurringExpense.end_date}`}
        ...
        <div className="row" style={{ alignItems: "center", gap: 8 }}>
          ...
          <button
            ...
            onClick={() => setDeleteRecurringTarget(recurringExpense)}
          >
            {T("btn_delete")}
          </button>
        </div>
