...
export function PacSection({ setDeletePacTarget }) {
  const {
    T,
    assets,
  ...
  return (
    <div>
      ...
      <div className="row" style={{ gap: 8, marginBottom: 16 }}>
        <button className="btn btn-p btn-sm" onClick={() => openPacModal()}>
          + {T("add_pac")}
        </button>
        <button
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {recurringInvestmentPlans.map((plan) => (
            <PacPlanRow
              ...
              T={T}
              plan={plan}
              target={assets.find((asset) => asset.id === plan.asset)}
              source={assets.find((asset) => asset.id === plan.source_account)}
              pacSaving={pacSaving}
      ...
      <div className="between">
        <div>
          <div style={{ fontSize: 14, fontWeight: 500 }}>
            {target?.investment_type_detail?.icon || "📈"} {plan.name}
          </div>
          <div
            style={{
              fontSize: 11,
        ...
        <div className="row" style={{ alignItems: "center", gap: 8 }}>
          ...
          >
            {plan.status === "ACTIVE" ? T("btn_disable") : T("btn_enable")}
          </button>
          <button
