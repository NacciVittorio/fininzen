      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div>
          ...
          <input
            ...
            value={pacForm.name}
            onChange={(event) =>
              setPacForm((state) => ({ ...state, name: event.target.value }))
            }
          />
        </div>
        <div>
          ...
          <select
            ...
            value={pacForm.asset}
            onChange={(event) =>
              setPacForm((state) => ({ ...state, asset: event.target.value }))
            }
          >
            <option value="">{T("select_asset")}</option>
            {investments
              .filter(
                (asset) => asset.tracking_type === "AUTO" && !asset.is_archived,
              )
              .map((asset) => (
                <option key={asset.id} value={asset.id}>
                  {asset.investment_type_detail?.icon || ""} {asset.name}
                </option>
              ))}
          </select>
        </div>
          ...
          >
            ...
            {bankAccounts.map((account) => (
              <option key={account.id} value={account.id}>
                {account.investment_type_detail?.icon || ""} {account.name}
              </option>
            ))}
          </select>
        </div>
        ...
        <PacCheckbox
          ...
          onChange={(checked) =>
            setPacForm((state) => ({ ...state, is_active: checked }))
          }
        >
          {T("recurring_active")}
        </PacCheckbox>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button
            ...
            onClick={submitPac}
          >
            {pacSaving ? "..." : editingPacId ? T("btn_update") : T("btn_add")}
          </button>
        </div>
      </div>
    </Modal>
...
function PacFrequencyFields({ T, MONTHS, pacForm, setPacForm }) {
  return (
    <>
      <div>
        ...
        <select
          ...
          onChange={(event) =>
            setPacForm((state) => ({
              ...
              anchor_month: ["QUARTERLY", "SEMIANNUAL", "ANNUAL"].includes(
                event.target.value,
              )
                ? state.anchor_month || String(new Date().getMonth() + 1)
                : "",
            }))
          }
        >
      ...
      {["QUARTERLY", "SEMIANNUAL", "ANNUAL"].includes(pacForm.frequency) && (
        <div>
...
function PacCheckbox({ checked, onChange, children }) {
  return (
    <label
      style={{
        display: "flex",
