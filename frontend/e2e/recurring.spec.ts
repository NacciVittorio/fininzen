...
async function createBankAccount(
    ...
    const accountRes = await page.request.post("/api/portfolio/", {
        headers: { Authorization: `Bearer ${token}` },
        data: {
    ...
    test("creates, edits, toggles, and deletes a recurring expense", async ({
        ...
        try {
            ...
            await page
                ...
                .first()
                .click();
            await modal.locator("select.inp").selectOption(String(accountId));
            ...
            await Promise.all([
                page.waitForResponse(
                    (r) =>
                        ...
                        r.url().includes("/api/expenses/recurring/") &&
                        r.ok(),
                ),
            ...
            const listRes = await page.request.get("/api/expenses/recurring/", {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            const recRes = await page.request.get("/api/expenses/recurring/", {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (recRes.ok()) {
            ...
            await page.request.delete(`/api/portfolio/${accountId}/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            await page.request.delete(
