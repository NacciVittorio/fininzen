...
async function createInvestmentType(page: Page, token: string): Promise<{ id: number; name: string }> {
  const name = `E2E Type ${Date.now()}`;
  const res = await page.request.post('/api/portfolio/investment-types/', {
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    data: { name, color: '#4ade80', icon: '🧪', supports_ticker: false, is_bank_account: false },
  });
  if (!res.ok()) throw new Error(`investment-type create failed: ${res.status()} — ${await res.text()}`);
  const body = await res.json();
  return { id: body.id, name };
}

async function deleteInvestmentType(page: Page, token: string, id: number): Promise<void> {
  await page.request.delete(`/api/portfolio/investment-types/${id}/`, {
    headers: { Authorization: `Bearer ${token}` },
  });
}

async function deleteAsset(page: Page, token: string, id: number): Promise<void> {
...
test.describe('Portfolio CRUD', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.click('button.nav-item:has-text("Investments")');
  ...
  test('add MANUAL asset appears in list', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"]').getByText('New Asset')).toBeVisible({ timeout: 5000 });
    ...
    await Promise.all([
      page.waitForResponse((r) => r.url().includes('/api/portfolio/') && r.ok()),
      page.click('[role="dialog"] button.btn.btn-p'),
    ]);

    // Asset visible in list
    await expect(page.locator(`text=${assetName}`)).toBeVisible({ timeout: 8000 });

    // Cleanup: get asset id then delete
    const assetsRes = await page.request.get('/api/portfolio/', {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  test('existing asset visible in investments list', async ({ page }) => {
    ...
    const res = await page.request.post('/api/portfolio/', {
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      data: {
        name: assetName,
