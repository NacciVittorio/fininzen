...
{ pkgs, ... }:
{
  ...
  enterShell = ''
    # The pip-installed ruff wheel is a generic glibc binary that can't run on
    # NixOS. Replace it with a symlink to the Nix-patched ruff so that PATH
    # lookup (used by pre-commit hooks and just lint) finds the working binary.
    ln -sf ${pkgs.ruff}/bin/ruff .devenv/state/venv/bin/ruff
