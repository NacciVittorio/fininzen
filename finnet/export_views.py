"""
finnet/export_views.py — Feature F (Data Export).
...
from finnet.mixins import _effective_user, resolve_view_as
from finnet.permissions import DEMO_USERNAME
...
def _zip_response(user, today_str):
    ...
    try:
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for kind, producer in _ZIP_PRODUCERS.items():
                with zf.open(f"finnet_{kind}_{today_str}.csv", "w") as raw_csv:
    ...
    response["Content-Disposition"] = _content_disposition(
        f"finnet_export_{today_str}.zip"
    )
    return response
...
class ExportView(APIView):
    ...
    def get(self, request):
        ...
        filename = f"finnet_{export_type}_{today_str}.csv"
