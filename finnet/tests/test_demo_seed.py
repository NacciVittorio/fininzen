...
from finnet.demo_seed import DEMO_EMAIL, DEMO_SEED_STATE_KEY, DEMO_SEED_VERSION
...
def _seed_calls(monkeypatch):
    ...
    monkeypatch.setattr("finnet.demo_seed.demo_seed_is_complete", lambda user: True)
    return calls
...
def test_demo_login_reseeds_when_dataset_is_incomplete(client, monkeypatch):
    ...
    monkeypatch.setattr("expenses.services.seed_demo_for_user", fake_seed)
    monkeypatch.setattr("finnet.demo_seed.demo_seed_is_complete", lambda user: False)
    DemoSeedState.objects.update_or_create(
        key=DEMO_SEED_STATE_KEY,
        defaults={
