...
import finnet.export_views as export_views
...
def test_export_accounts_returns_csv(client, bank_account, etf_asset):
    ...
    assert 'filename="finnet_accounts_' in res["Content-Disposition"]
    assert "filename*=UTF-8''finnet_accounts_" in res["Content-Disposition"]
...
def test_export_all_returns_zip_with_every_kind(
    ...
    assert 'filename="finnet_export_' in res["Content-Disposition"]
    assert "filename*=UTF-8''finnet_export_" in res["Content-Disposition"]
    with zipfile.ZipFile(io.BytesIO(_response_body(res))) as zf:
        names = zf.namelist()
        for kind in ("accounts", "assets", "transactions", "cashflow", "price_history"):
            assert any(
                n.startswith(f"finnet_{kind}_") and n.endswith(".csv") for n in names
            ), f"missing {kind} in zip: {names}"
        tx_name = next(n for n in names if n.startswith("finnet_transactions_"))
